...
const CatalogView: React.FC<CatalogViewProps> = ({ items, onAddToCart, cartItemIds, wishlist, onToggleWishlist, onEditItem, onDeleteItem, isAdminMode }) => {
  ...
  return (
    <div className="animate-fade-in pb-24">
      <div className="mb-6 flex items-center justify-center flex-wrap gap-2 sticky top-20 z-10 bg-brand-background/95 py-2">
        {CATEGORIES.map(category => (
          <button
            key={category}
            onClick={() => setSelectedCategory(category)}
            className={`px-4 py-2 rounded-full text-sm font-bold transition-colors duration-200 shadow-sm ${
              selectedCategory === category
                ? 'bg-brand-accent text-brand-white-ish'
                : 'bg-brand-white-ish text-brand-text hover:bg-brand-primary hover:text-white'
            }`}
          >
            {category}
          </button>
      ...
      {filteredItems.length > 0 ? (
        // Updated Grid: 1 col on mobile, 2 on small tablets, 3 on normal tablets, 4 on desktop
